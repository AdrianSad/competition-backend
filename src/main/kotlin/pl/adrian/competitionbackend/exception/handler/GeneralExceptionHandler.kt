package pl.adrian.competitionbackend.exception.handler

import pl.adrian.competitionbackend.exception.model.DomainException
import pl.adrian.competitionbackend.exception.model.DomainExceptionDto
...
class GeneralExceptionHandler(private val messageSource: MessageSource) : ResponseEntityExceptionHandler() {
    ...
    fun handleDomainException(
            exception: DomainException): ResponseEntity<DomainExceptionDto> {
        val response = DomainExceptionDto()
        response.code = exception.code
        response.message = exception.message
        response.status = exception.status
        val localizedMessage = messageSource.getMessage(exception.code, null, Utils.polishLocale)
        response.localizedMessage = localizedMessage
